...
public final class TerminalBuffer {
    ...
    public String getSelectedText(int selX1, int selY1, int selX2, int selY2, boolean joinBackLines) {
        ...
        for (int row = selY1; row <= selY2; row++) {
            ...
            int i;
            boolean rowLineWrap = getLineWrap(row) || x2 == columns;
            if (rowLineWrap) {
    ...
    public String getWordAtLocation(int x, int y) {
        int y1 = y;
        int y2 = y;
        while (y1 > 0 && !getSelectedText(0, y1 - 1, mColumns, y2).contains("\n")) {
            y1--;
        }
        while (y2 < mScreenRows && !getSelectedText(0, y1, mColumns, y2 + 1).contains("\n")) {
            y2++;
        }
        String text = getSelectedText(0, y1, mColumns, y2);
        x += (y - y1) * mColumns;
        int x1 = text.lastIndexOf(' ', x);
        if (x1 == -1) {
            x1 = 0;
        }
        int x2 = text.indexOf(' ', x);
        if (x2 == -1) {
            x2 = text.length();
        }
